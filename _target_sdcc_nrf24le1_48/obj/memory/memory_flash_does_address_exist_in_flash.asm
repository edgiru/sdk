...
;--------------------------------------------------------
	.module memory_flash_does_address_exist_in_flash
...
;--------------------------------------------------------
	.area XSEG    (XDATA)
_memory_flash_does_address_exist_in_flash_address_1_14:
	.ds 2
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'memory_flash_does_address_exist_in_flash'
;------------------------------------------------------------
;address                   Allocated with name '_memory_flash_does_address_exist_in_flash_address_1_14'
;------------------------------------------------------------
;	src/memory_flash_does_address_exist_in_flash.c:47: bool memory_flash_does_address_exist_in_flash(uint16_t address)
;	-----------------------------------------
;	 function memory_flash_does_address_exist_in_flash
;	-----------------------------------------
_memory_flash_does_address_exist_in_flash:
	ar7 = 0x07
	ar6 = 0x06
	ar5 = 0x05
	ar4 = 0x04
	...
	mov	r7,dph
	mov	a,dpl
	mov	dptr,#_memory_flash_does_address_exist_in_flash_address_1_14
	movx	@dptr,a
	mov	a,r7
	inc	dptr
	movx	@dptr,a
;	src/memory_flash_does_address_exist_in_flash.c:50: if(address <= MEMORY_FLASH_CODE_END_ADDRESS) //Don't need to check start, as it's 0 and address type is unsigned
	mov	dptr,#_memory_flash_does_address_exist_in_flash_address_1_14
	movx	a,@dptr
	mov	r6,a
	inc	dptr
	...
	subb	a,r7
	jc	00108$
;	src/memory_flash_does_address_exist_in_flash.c:52: return true;
	mov	dpl,#0x01
	ret
00108$:
;	src/memory_flash_does_address_exist_in_flash.c:54: else if((address >= MEMORY_FLASH_NV_EXT_END_START_ADDRESS) && (address <= MEMORY_FLASH_NV_EXT_END_END_ADDRESS))
	mov	a,#0x100 - 0xFA
	add	a,r7
	jnc	00104$
	clr	c
	...
	subb	a,r7
	jc	00104$
;	src/memory_flash_does_address_exist_in_flash.c:56: return true;
	mov	dpl,#0x01
	ret
00104$:
;	src/memory_flash_does_address_exist_in_flash.c:58: else if(address >= MEMORY_FLASH_NV_STD_END_START_ADDRESS) //Don't need to check end, as it's 65535 and address type is uint16_t
	mov	a,#0x100 - 0xFC
	add	a,r7
	jnc	00109$
;	src/memory_flash_does_address_exist_in_flash.c:60: return true;
	mov	dpl,#0x01
	ret
00109$:
;	src/memory_flash_does_address_exist_in_flash.c:63: return false;
	mov	dpl,#0x00
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
