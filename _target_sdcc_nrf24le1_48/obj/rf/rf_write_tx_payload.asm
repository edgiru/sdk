...
; This file was generated Sun Jul 20 08:47:53 2014
;--------------------------------------------------------
...
_rf_write_tx_payload_PARM_3:
	.ds 1
_rf_write_tx_payload_dataptr_1_31:
	.ds 3
;--------------------------------------------------------
...
;dataptr                   Allocated with name '_rf_write_tx_payload_dataptr_1_31'
;status                    Allocated with name '_rf_write_tx_payload_status_1_32'
;------------------------------------------------------------
...
_rf_write_tx_payload:
	...
	mov	r6,dph
	mov	a,dpl
	mov	dptr,#_rf_write_tx_payload_dataptr_1_31
	movx	@dptr,a
	mov	a,r6
	inc	dptr
	movx	@dptr,a
	...
	inc	dptr
	movx	@dptr,a
;	src/rf_write_tx_payload.c:53: status = rf_spi_execute_command(RF_W_TX_PAYLOAD, dataptr, len, false); //Write the payload
	mov	dptr,#_rf_write_tx_payload_dataptr_1_31
	movx	a,@dptr
	mov	r5,a
	inc	dptr
	...
	mov	r7,dpl
;	src/rf_write_tx_payload.c:56: if(transmit)
	mov	dptr,#_rf_write_tx_payload_PARM_3
	movx	a,@dptr
	mov	r6,a
	jz	00104$
;	src/rf_write_tx_payload.c:58: if(!rf_get_ce())
	jb	_RFCON_SB_RFCE,00105$
;	src/rf_write_tx_payload.c:60: rf_transmit();
	push	ar7
	lcall	_rf_transmit
	pop	ar7
	sjmp	00105$
00104$:
;	src/rf_write_tx_payload.c:65: rf_clear_ce();
	clr _RFCON_SB_RFCE 
00105$:
;	src/rf_write_tx_payload.c:68: return status; //Return the value of STATUS
	mov	dpl,r7
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
