...
;--------------------------------------------------------
	.area XSEG    (XDATA)
_rf_set_output_power_power_1_31:
	.ds 1
_rf_set_output_power_rf_setup_2_33:
	.ds 1
;--------------------------------------------------------
...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'rf_set_output_power'
;------------------------------------------------------------
;power                     Allocated with name '_rf_set_output_power_power_1_31'
;rf_setup                  Allocated with name '_rf_set_output_power_rf_setup_2_33'
;------------------------------------------------------------
;	src/rf_set_output_power.c:47: void rf_set_output_power(uint8_t power)
;	-----------------------------------------
;	 function rf_set_output_power
;	-----------------------------------------
_rf_set_output_power:
	ar7 = 0x07
	ar6 = 0x06
	ar5 = 0x05
	ar4 = 0x04
	...
	ar0 = 0x00
	mov	a,dpl
	mov	dptr,#_rf_set_output_power_power_1_31
	movx	@dptr,a
;	src/rf_set_output_power.c:49: if (power <= RF_RF_SETUP_RF_PWR_0_DBM) {
	movx	a,@dptr
	mov  r7,a
	add	a,#0xff - 0x06
	jc	00103$
;	src/rf_set_output_power.c:50: uint8_t rf_setup = rf_read_register_1_byte(RF_RF_SETUP);
	mov	dpl,#0x06
	push	ar7
	lcall	_rf_read_register_1_byte
	mov	r6,dpl
	pop	ar7
	mov	dptr,#_rf_set_output_power_rf_setup_2_33
	mov	a,r6
	movx	@dptr,a
;	src/rf_set_output_power.c:51: rf_setup = (rf_setup & ~RF_RF_SETUP_RF_PWR) | power;
	anl	ar6,#0xF9
	mov	dptr,#_rf_set_output_power_rf_setup_2_33
	mov	a,r7
	orl	a,r6
	movx	@dptr,a
;	src/rf_set_output_power.c:52: rf_write_register(RF_RF_SETUP, &rf_setup, 1);
	mov	dptr,#_rf_write_register_PARM_2
	mov	a,#_rf_set_output_power_rf_setup_2_33
	movx	@dptr,a
	mov	a,#(_rf_set_output_power_rf_setup_2_33 >> 8)
	inc	dptr
	movx	@dptr,a
	clr	a
	inc	dptr
