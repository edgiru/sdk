...
_rf_power_up_param_PARM_2:
	.ds 1
_rf_power_up_param_rx_active_mode_1_37:
	.ds 1
;--------------------------------------------------------
...
;rx_active_mode            Allocated with name '_rf_power_up_param_rx_active_mode_1_37'
;------------------------------------------------------------
...
_rf_power_up_param:
	...
	ar0 = 0x00
	mov	a,dpl
	mov	dptr,#_rf_power_up_param_rx_active_mode_1_37
	movx	@dptr,a
...
;	src/rf_power_up_param.c:56: while(!pwr_clk_mgmt_is_cclk_src_xosc16m()); //wait for the clock to get switched to the 16 MHz crystal, otherwise the RF can't be run
00101$:
	mov	a,_CLKLFCTRL
	jnb	acc.3,00101$
;	src/rf_power_up_param.c:59: if((config & RF_CONFIG_PRIM_RX) == 0)
	mov	dptr,#_rf_power_up_param_PARM_2
	movx	a,@dptr
	mov	r7,a
	jb	acc.0,00110$
;	src/rf_power_up_param.c:61: rf_clear_ce(); //Clear CE for TX mode
	clr _RFCON_SB_RFCE 
	sjmp	00111$
00110$:
;	src/rf_power_up_param.c:66: if(rx_active_mode)
	mov	dptr,#_rf_power_up_param_rx_active_mode_1_37
	movx	a,@dptr
	mov	r7,a
	jz	00107$
;	src/rf_power_up_param.c:69: if(!rf_get_ce())
	jb	_RFCON_SB_RFCE,00111$
;	src/rf_power_up_param.c:71: rf_set_ce();
	setb _RFCON_SB_RFCE 
	mov	dptr,#0x0004
	lcall	_delay_us
	sjmp	00111$
00107$:
;	src/rf_power_up_param.c:76: rf_clear_ce();
	clr _RFCON_SB_RFCE 
00111$:
;	src/rf_power_up_param.c:80: delay_us(1); //Delay for power up
	mov	dptr,#0x0001
	ljmp	_delay_us
	.area CSEG    (CODE)
	.area CONST   (CODE)
