...
; This file was generated Sun Jul 20 08:46:05 2014
;--------------------------------------------------------
	.module rf_set_as_tx
...
;------------------------------------------------------------
;config                    Allocated with name '_rf_set_as_tx_config_1_31'
;ce_status                 Allocated with name '_rf_set_as_tx_ce_status_1_31'
;------------------------------------------------------------
...
_rf_set_as_tx:
	...
	ar1 = 0x01
	ar0 = 0x00
;	src/rf_set_as_tx.c:50: uint8_t ce_status = rf_get_ce();
	mov	c,_RFCON_SB_RFCE
	clr	a
	rlc	a
	mov	r7,a
;	src/rf_set_as_tx.c:52: rf_read_register(RF_CONFIG, &config, 1); //Read the current CONFIG value
	mov	dptr,#_rf_read_register_PARM_2
	mov	a,#_rf_set_as_tx_config_1_31
	movx	@dptr,a
	mov	a,#(_rf_set_as_tx_config_1_31 >> 8)
	inc	dptr
	movx	@dptr,a
	clr	a
	inc	dptr
	...
	pop	ar7
;	src/rf_set_as_tx.c:55: if((config & RF_CONFIG_PRIM_RX) == 0)
	mov	dptr,#_rf_set_as_tx_config_1_31
	movx	a,@dptr
	mov	r6,a
	jb	acc.0,00102$
;	src/rf_set_as_tx.c:57: return;
	ret
00102$:
;	src/rf_set_as_tx.c:60: rf_clear_ce(); //Clear the CE pin to enter TX mode
	clr _RFCON_SB_RFCE 
;	src/rf_set_as_tx.c:63: config &= (~RF_CONFIG_PRIM_RX);
	mov	dptr,#_rf_set_as_tx_config_1_31
	mov	a,#0xFE
	anl	a,r6
	movx	@dptr,a
;	src/rf_set_as_tx.c:64: rf_write_register(RF_CONFIG, &config, 1);
	...
	pop	ar7
;	src/rf_set_as_tx.c:67: if(ce_status)
	mov	a,r7
	jz	00105$
;	src/rf_set_as_tx.c:69: rf_set_ce();
	setb _RFCON_SB_RFCE 
	mov	dptr,#0x0004
	ljmp	_delay_us
00105$:
