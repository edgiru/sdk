...
;--------------------------------------------------------
	.area CSEG    (CODE)
;------------------------------------------------------------
;Allocation info for local variables in function 'rf_irq_clear'
;------------------------------------------------------------
;irq_mask                  Allocated with name '_rf_irq_clear_irq_mask_1_31'
;------------------------------------------------------------
;	src/rf_irq_clear.c:47: void rf_irq_clear(uint8_t irq_mask)
;	-----------------------------------------
;	 function rf_irq_clear
;	-----------------------------------------
_rf_irq_clear:
	ar7 = 0x07
	ar6 = 0x06
	...
	ar0 = 0x00
	mov	a,dpl
	mov	dptr,#_rf_irq_clear_irq_mask_1_31
	movx	@dptr,a
;	src/rf_irq_clear.c:50: rf_write_register(RF_STATUS, &irq_mask, 1);
	mov	dptr,#_rf_write_register_PARM_2
	mov	a,#_rf_irq_clear_irq_mask_1_31
	movx	@dptr,a
	mov	a,#(_rf_irq_clear_irq_mask_1_31 >> 8)
	inc	dptr
	movx	@dptr,a
	clr	a
	inc	dptr
...
;	src/rf_irq_clear.c:52: sbit_clear(IRCON_SB_RFIRQ); //Clear the master interrupt
	clr _IRCON_SB_RFIRQ 
	ret
	.area CSEG    (CODE)
	.area CONST   (CODE)
